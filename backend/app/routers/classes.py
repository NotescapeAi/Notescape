...
async def create_class(
    ...
    logging.info(f"Creating class for user {user_uid}: {payload.name}")
    try:
        ...
        logging.info(f"Class created successfully: {result}")
        return result
    except Exception as e:
        logging.error(f"Error creating class: {e}", exc_info=True)
...
@router.get("/{class_id}/documents/{document_id}/view")
async def view_document(
    ...
    return FileResponse(file_path, media_type=mime_type, filename=filename, content_disposition_type="inline")
...
async def get_document_view_url(
    ...
    url = f"/api/classes/{class_id}/documents/{document_id}/view"
    return {"url": url, "content_type": mime_type}
