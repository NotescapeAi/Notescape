...
async def run():
    ...
    while True:
        ...
        try:
            ...
            for it in items:
                src = it.get("source")
                if src and src.get("chunk_id") not in valid_chunk_ids:
                    # Log warning and remove invalid chunk_id to prevent FK violation
                    log.warning(f"[quiz_worker] Invalid chunk_id {src.get('chunk_id')} generated by LLM. Removing source link.")
                    src["chunk_id"] = None
            ...
            msg = "Something went wrong while generating quiz. Please try again."
            # Allow specific user-friendly errors to pass through
